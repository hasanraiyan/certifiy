 ...
 */
export function validateExamConfig(config: unknown): config is ExamConfig {
  ...
  if (!c.type || !['practice', 'test'].includes(c.type as string)) {
    throw new ValidationError('ExamConfig.type must be "practice" or "test"', 'type');
  }

  if (!c.examType || !['full-mock', 'domain-quiz', 'knowledge-area'].includes(c.examType as string)) {
 ...
 */
export function validateExamSettings(settings: unknown): settings is ExamSettings {
...
export function validateQuestion(question: unknown): question is Question {
  ...
  if (!q.type || !['multiple-choice', 'multiple-select', 'true-false'].includes(q.type as string)) {
 ...
 */
export function validateAnswer(answer: unknown): answer is Answer {
 ...
 */
export function sanitizeAnswer(answer: Answer): Answer {
  return {
...
export function validateAnswerCompleteness(
  ...
  questions: Question[]
): { isComplete: boolean; missingQuestions: number[] } {
...
export function validateExamSession(session: unknown): session is ExamSession {
  ...
  if (!s.status || !['setup', 'in-progress', 'review', 'completed', 'abandoned'].includes(s.status as string)) {
 ...
 */
export function validateExamResults(results: unknown): results is ExamResults {
 ...
 */
export function validateDomainScore(domainScore: unknown): domainScore is DomainScore {
  if (!domainScore || typeof domainScore !== 'object') {
    throw new ValidationError('DomainScore must be an object');
  }

  const d = domainScore as Record<string, unknown>;
 ...
 */
export function validateQuestionResult(questionResult: unknown): questionResult is QuestionResult {
  if (!questionResult || typeof questionResult !== 'object') {
    throw new ValidationError('QuestionResult must be an object');
  }

  const q = questionResult as Record<string, unknown>;
