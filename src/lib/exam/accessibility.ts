...
export const ariaLabels = {
  question: (number: number, total: number, isAnswered: boolean, isBookmarked: boolean) =>
    `Question ${number} of ${total}${isAnswered ? ', answered' : ', not answered'}${isBookmarked ? ', bookmarked' : ''}`,
  
  questionOption: (optionIndex: number, optionText: string, isSelected: boolean, questionType: string) =>
    `${questionType === 'multiple-select' ? 'Checkbox' : 'Radio button'} option ${optionIndex + 1}: ${optionText}${isSelected ? ', selected' : ''}`,
  
  navigationButton: (questionNumber: number, status: string, isBookmarked: boolean) =>
  ...
  timer: (timeRemaining: string, isTestMode: boolean) =>
  ...
  submitExam: (answered: number, total: number) =>
    `Submit exam: ${answered} of ${total} questions answered`,
  
  reviewMode: (unanswered: number) =>
...
export class ScreenReaderAnnouncer {
  ...
  announceQuestionChange(questionNumber: number, totalQuestions: number, isAnswered: boolean) {
    this.announce(
