...
export function useExamPersistence(options = {}) {
  const {
    autoCleanup = true,
  ...
  const getSessionIndex = useCallback(() => {
    try {
  ...
  const updateSessionIndex = useCallback((sessionId, remove = false) => {
    try {
  ...
  const saveSession = useCallback(async (session, questions) => {
  ...
  const loadResults = useCallback(async (sessionId) => {
    ...
    try {
      ...
      const serializedResults = JSON.parse(resultsData);
  ...
  const listSavedSessions = useCallback(async () => {
    setIsLoading(true);
    clearError();
  ...
  const findSessionsByUser = useCallback(async (userId) => {
    const allSessions = await listSavedSessions();
    return allSessions.filter(session => session.userId === userId);
  }, [listSavedSessions]);

  const cleanupExpiredSessions = useCallback(async (maxAgeHours = maxSessionAge) => {
    setIsLoading(true);
    clearError();
  ...
  const cleanupCompletedSessions = useCallback(async (keepRecentHours = DEFAULT_KEEP_COMPLETED_HOURS) => {
    setIsLoading(true);
    clearError();
  ...
  const validateStoredSession = useCallback(async (sessionId) => {
    try {
  ...
  const repairSession = useCallback(async (sessionId) => {
  ...
  const exportSessionData = useCallback(async (sessionId) => {
    try {
  ...
  const importSessionData = useCallback(async (data) => {
    try {
